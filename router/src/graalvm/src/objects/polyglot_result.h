...
#include "router/src/graalvm/src/objects/polyglot_row.h"
#include "router/src/graalvm/src/utils/native_value.h"

namespace shcore {
namespace polyglot {

class Result : public Object_bridge {
 public:
  explicit Result(const std::shared_ptr<graalvm::db::IResult> &result);
 ...
 private:
  ...
  std::shared_ptr<graalvm::db::IResult> m_result;

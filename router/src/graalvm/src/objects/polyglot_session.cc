...
Session::Session(const std::shared_ptr<graalvm::db::ISession> &session)
    : m_session{session} {}
...
std::shared_ptr<Result> Session::run_sql(const Argument_list &args) {
  auto query = args[0].as_string();
  if (args.size() > 1) {
    query = sub_query_placeholders(query, args[1].as_array());
  }

  return std::make_shared<Result>(m_session->run_sql(query));
}

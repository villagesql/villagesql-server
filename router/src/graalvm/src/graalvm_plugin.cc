...
#include "mysqlrouter/graalvm_options.h"
...
static constexpr std::string_view kSectionName{"graalvm"};
static constexpr uint32_t kDefaultMaxGraalVMContexts{8};

class GraalVMPluginConfig : public mysql_harness::BasePluginConfig {
 public:
  uint32_t max_contexts;

  explicit GraalVMPluginConfig(const mysql_harness::ConfigSection *section)
...
static void init(mysql_harness::PluginFuncEnv *env) {
  ...
  // auto &graalvm_component = GraalVMComponent::get_instance();
  ...
  try {
    ...
    for (const mysql_harness::ConfigSection *section :
      ...
      GraalVMPluginConfig config{section};
    }
...
class GraalVMConfigExposer : public mysql_harness::SectionConfigExposer {
 public:
  using DC = mysql_harness::DynamicConfig;
  GraalVMConfigExposer(bool initial, const GraalVMPluginConfig &plugin_config,
 ...
 private:
  const GraalVMPluginConfig &plugin_config_;
};
...
static void expose_configuration(mysql_harness::PluginFuncEnv *env,
  ...
  for (const mysql_harness::ConfigSection *section : info->config->sections()) {
    if (section->name == kSectionName) {
      GraalVMPluginConfig config{section};
      GraalVMConfigExposer(initial, config, info->config->get_default_section())
          .expose();
    }
  }
}
...
extern "C" {
mysql_harness::Plugin GRAALVM_PLUGIN_EXPORT harness_plugin_graalvm = {

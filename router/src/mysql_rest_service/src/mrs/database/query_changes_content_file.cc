...
void QueryChangesContentFile::query_entries(MySQLSession *session) {
  QueryAuditLogEntries audit_entries;
  VectorOfPaths local_entries;
...
void QueryChangesContentFile::query_content_file_entries(
  ...
  if (entries.empty() && table_name == "content_file") {
    entry::ContentFile pe;
    pe.id = id;
    pe.deleted = true;

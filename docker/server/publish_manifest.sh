...
publish_manifest() {
    local manifest_args=() version_label base_tag manifest_tag
    ...
    if [ "${#MANIFEST_TAGS_LIST[@]}" -gt 0 ]; then
        for manifest_tag in "${MANIFEST_TAGS_LIST[@]}"; do
            manifest_args+=(-t "$REPO:${manifest_tag}")
        done
    fi
...
main() {
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            -m|--manifest-tags)      MANIFEST_TAGS="$2"; shift 2 ;;
    ...
    split_list "$MANIFEST_TAGS"
    MANIFEST_TAGS_LIST=("${SPLIT_RESULT[@]+"${SPLIT_RESULT[@]}"}")

    echo "Repository     : $REPO"
    echo "Arch images    : ${ARCH_TAGS[*]}"
    echo "Image tag      : $TAG"
    echo "Version labels : ${VERSION_LABELS_LIST[*]+${VERSION_LABELS_LIST[*]}}"
    echo "Manifest tags  : ${MANIFEST_TAGS_LIST[*]+${MANIFEST_TAGS_LIST[*]}}"

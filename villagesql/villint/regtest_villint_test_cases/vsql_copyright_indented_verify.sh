#!/bin/bash
# Verify script for vsql_copyright_indented test case
...
if [ "$ORACLE_INDENT" = "$VILLAGESQL_INDENT" ]; then
    ...
    echo "  Oracle indent: '$ORACLE_INDENT' (length: ${#ORACLE_INDENT})"
    echo "  VillageSQL indent: '$VILLAGESQL_INDENT' (length: ${#VILLAGESQL_INDENT})"
    exit 0
else

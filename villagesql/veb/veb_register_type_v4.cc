...
std::optional<TypeDescriptor> build_type_descriptor_v4(
  ...
  if (td->persisted_length == -1) {
    // Variable-length type: max_persisted_length bounds the field and is always
    // required. resolve_params is required only for parameterized variants
    // (those that also expose int_to_params); a bare variable-length type whose
    // length is decided per value needs only max_persisted_length.

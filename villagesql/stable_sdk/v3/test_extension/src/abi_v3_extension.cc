...
// ABI v3 Test Extension
//
// Frozen snapshot of vsql_test_only compiled against the stable_sdk/v3/
// headers. Verifies that extensions built against v3 headers continue to load
// and run against the current server.
//
...
// PVEC(N): parameterized vector of N int16 values.
//
...
void pvec_concat_bind(vsql::BindArgs args, vsql::BindResult out) {
  if (args.size() != 2) {
    out.error("pvec_concat expects 2 arguments");
    return;
  }
  const PVecParams *a = args.at(0).params<PVecParams>();
  const PVecParams *b = args.at(1).params<PVecParams>();
  if (a == nullptr || b == nullptr) {
    out.error("pvec_concat: both arguments need a known dimension");
    return;
  }
  out.set_return(PVecParams{a->dimension + b->dimension});
}
...
// fault_blob_hash: FNV-1a hash of the stored bytes. Covers TypeHashFunc and
// the .hash() type builder option.
static size_t fault_blob_hash(vsql::CustomArg in) {
  auto data = in.value();
...
// real_double: doubles a REAL value; NULL input -> NULL output.
// Covers RealArg, RealResult, RealResult::set(), RealResult::set_null().
// Registered with .deterministic(true) to cover that builder option.
static void real_double(vsql::RealArg x, vsql::RealResult out) {
  if (x.is_null()) {
    out.set_null();
    return;
  }
...
// answer: zero-arity function. Covers the .no_params() builder method.
static void answer(vsql::IntResult out) { out.set(42); }
...
VEF_GENERATE_ENTRY_POINTS(
    make_extension()
        ...
        .func(make_func<&real_double>("real_double")
                  ...
                  .deterministic(true)
                  .build())
        // answer: covers .no_params() zero-arity function
        .func(make_func<&answer>("answer").returns(INT).no_params().build()))

...
class FuncBuilder {
 public:
  ...
  // compute the return type's parameters, bypassing the built-in TD1/TD2
  ...
  // vector_concat(vector(M), vector(N)) -> vector(M+N)) or when the return
  // type's parameters come from a constant argument value (e.g.

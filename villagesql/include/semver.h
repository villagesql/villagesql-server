...
/**
 * A semantic version, as defined by the semver.org specification.
 ...
 *
 * A VillageSQL build or schema version additionally names the code base it
 * derives from; that pairing lives in CodeBaseVersion (see
 * villagesql/include/code_base_version.h), which holds one of these.
 */
class Semver {
 public:
   ...
   * @return String in semver format, or "" if this version is invalid
   */
   ...
   * An invalid version compares neither equal to, less than, nor greater than
   * anything, including another invalid version.
   */

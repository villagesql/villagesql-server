...
static void vef_sys_var_update_trampoline(MYSQL_THD, SYS_VAR *, void *val_ptr,
  ...
  // g_sys_vars_mutex. If UNINSTALL EXTENSION races with a concurrent SET GLOBAL,
  // on_depopulate_sys_var may erase the entry and return (allowing dlclose)
  // while this thread holds the captured on_change pointer but has already
  // released the lock. A drain mechanism analogous to statement_event's

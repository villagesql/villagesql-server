  ...
  if (!is_numeric(version_str)) {
    if (error)
      *error = std::string(name) + " must be numeric, not " +
               sanitize_for_output(version_str);
    return false;
  }
  ...
  if (ec != std::errc()) {
    if (error)
      *error = std::string(name) + " version (" +
               sanitize_for_output(version_str) + ") is out of range";
    return false;
  }
...
bool Semver::parse(std::string_view s, std::string *error) {
  ...
  *this = Semver();
  ...
  if (!from_components(major, minor, patch, prerelease, build_metadata)) {
    if (error && error->empty())
      *error = "Invalid version string: " + sanitize_for_output(s);
    return false;
  }
  return true;
}
...
// Defined in terms of < and == (not negation) so that an invalid version stays
// unordered against everything: all four relational operators are false.

...
// The vector is the sole key column, at key position 0.
...
static bool kvec_insert(IdxCtx *ctx, const Index &index,
  ...
  if (key_columns == nullptr || key_columns[0].data == nullptr) {
    snprintf(err, err_len, "kvec_store: missing key column");
    return true;
  }
  // Ask the server for the stable col_ref of this stored value (get_key_ref /
  // col_data_to_ref_fn) -- the ref is NOT assumed to be embedded in the key
  // bytes. This mirrors vsql-vector's create_node (get_key_ref), which the ABI
  // requires: the server computes the col_ref for a value in the column store.
  IndexScanKey::KeyPartRef ref;
  if (index.get_key_ref(kKeyPos, key_columns[0], &ref)) {
    snprintf(err, err_len, "kvec_store: get_key_ref failed: %s",
             index.get_error());
    return true;
  }
...
static bool kvec_begin(IdxCtx *ctx, const Index &index, MtrCtx::Ref /*mctx*/,
  ...
  *eof = true;
  ...
  // server writes the full persisted [8-byte ref][floats] value into it and sets
  // raw.length to the actual length. So stored is a complete VDF operand
  // matching the query key -- nothing to reassemble. Sized via get_max_col_len,
...
static void kvec_scan_begin_count(vsql::IntResult out) {
  out.set(g_scan_begin_count.load());
}
static void kvec_scan_fetch_count(vsql::IntResult out) {
  out.set(g_scan_fetch_count.load());
}
static void kvec_insert_count(vsql::IntResult out) {
  out.set(g_insert_count.load());
}
// Reset all observability counters to 0 (for a clean per-test baseline).
// Returns 0; the return exists only because a SQL function must return a value.
static void kvec_reset_counters(vsql::IntResult out) {
  g_scan_begin_count.store(0);
  g_scan_fetch_count.store(0);
  g_insert_count.store(0);
  out.set(0);
}
...
VEF_GENERATE_ENTRY_POINTS(
    make_extension()
        ...
        .func(make_func<&kvec_scan_fetch_count>("kvec_store_scan_fetch_count")
                  ...
                  .no_params()
                  .build())
        .func(make_func<&kvec_insert_count>("kvec_store_insert_count")
                  .returns(INT)
                  .no_params()
                  .build())
        .func(make_func<&kvec_reset_counters>("kvec_store_reset_counters")
                  .returns(INT)
                  .no_params()
                  .build()))

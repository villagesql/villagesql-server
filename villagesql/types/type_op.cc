...
String *EncodeOp::invoke(const String &from, int64_t buffer_hint,
                         MEM_ROOT &mem_root, bool &is_valid) const {
  is_valid = true;
  ...
  if (vdf_ != nullptr) {
    ...
    auto *buffer = new (&mem_root) uchar[buffer_size];
    ...
    vdf_result.max_bin_len = buffer_size;
    vdf_result.actual_len = 0;
    vdf_result.alt_bin_buf = &alt_bin_buf;
    ...
    const uchar *result_data = (alt_bin_buf != nullptr) ? alt_bin_buf : buffer;
    const size_t actual_len = vdf_result.actual_len;
    if (alt_bin_buf != nullptr) {
      buffer = new (&mem_root) uchar[actual_len];
      if (should_assert_if_null(buffer)) {
        my_error(ER_OUTOFMEMORY, MYF(ME_FATALERROR), actual_len);
        return nullptr;
      }
      memcpy(buffer, result_data, actual_len);
    }

    auto *ret = new (&mem_root)
        String(pointer_cast<const char *>(buffer), actual_len, &my_charset_bin);
  ...
  if (fn_(pointer_cast<uchar *>(buffer), buffer_hint, from.ptr(), from.length(),
          &actual_length)) {
    is_valid = false;
    return nullptr;
  }

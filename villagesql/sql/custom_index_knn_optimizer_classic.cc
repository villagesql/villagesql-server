...
bool IsCustomKnnDistanceOrderItem(TABLE *table, Item *order_item) {
  // Feature gate (see TrySkipSortWithCustomKnnIndex). When off, this is never a
  // KNN ordering, so report "not a KNN item" and let JOIN::optimize treat the
...
bool TrySkipSortWithCustomKnnIndex(JOIN_TAB *tab, ORDER *order,
  ...
  // work. This is also the quick escape that keeps the recognition cost off
  // test_if_skip_sort_order()'s hot path for every ordinary query.
  //
  // Returning here is what disables the whole classic custom-index path: this
  // function is the sole site that sets JT_INDEX_DISTANCE (below). Bailing
  // leaves the table's join type unchanged, so QEP_TAB::access_path() never
  // takes the JT_INDEX_DISTANCE branch and BuildCustomKnnDistanceAccessPath is

...
// An invalid version is unordered against everything, including another
// invalid version: all six operators agree it is not comparable.
TEST_F(SemverTest, InvalidVersionsAreUnordered) {
  const Semver invalid;
  const Semver valid = from_components(1, 0, 0);
  ...
  const Semver other_invalid;
  EXPECT_FALSE(invalid == other_invalid);
  EXPECT_FALSE(invalid < other_invalid);
}
...
TEST_F(SemverTest, FromComponentsInvalid) {
  ...
  // Core components exceeding their per-position bounds
  Semver v7 = from_components(Semver::kMajorMax + 1, 0, 0);
  EXPECT_FALSE(v7.is_valid());

  Semver v8 = from_components(0, Semver::kMinorMax + 1, 0);
  EXPECT_FALSE(v8.is_valid());

  Semver v9 = from_components(0, 0, Semver::kPatchMax + 1);
  EXPECT_FALSE(v9.is_valid());
}
...
TEST_F(SemverTest, ParseRejectsOutOfRangeComponent) {
  Semver v;
  std::string error;
  EXPECT_FALSE(v.parse("99999999999999999999999.0.0", &error));
  EXPECT_THAT(error, ::testing::HasSubstr("is out of range"));
  EXPECT_FALSE(v.is_valid());
}

TEST_F(SemverTest, PrereleasePrecedenceIsSymmetric) {
  const Semver num_low = from_components(1, 0, 0, {"1"});
  const Semver num_high = from_components(1, 0, 0, {"2"});
  const Semver alpha = from_components(1, 0, 0, {"alpha"});
  const Semver beta = from_components(1, 0, 0, {"beta"});

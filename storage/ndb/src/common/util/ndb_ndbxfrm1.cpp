...
ndb_ndbxfrm1::trailer::trailer()
    : m_file_pos(0), m_file_block_size(0), m_zero_pad_size(0) {
  memset(&m_buffer, 0, sizeof(m_buffer));
...
int ndb_ndbxfrm1::header::detect_header(const ndbxfrm_input_iterator *in,
  ...
  if (len < 8) return need_more_input;
  ...
  if (header_size < MIN_HEADER_SIZE) RETURN(-1);
  if (header_size % 8 != 0) RETURN(-1);
...
int ndb_ndbxfrm1::header::read_header(ndbxfrm_input_iterator *in) {
  ...
  if (header_size > len) RETURN(-1);
  if (fixed_header_size > header_size) RETURN(-1);
  Uint32 copy_size = fixed_header_size;
  while (copy_size > 0 && p[copy_size - 1] == 0) {
    copy_size--;
  }
  if (copy_size > sizeof(fixed_header)) RETURN(-1);
  memset(&m_buffer.m_header, 0, sizeof(m_buffer.m_header));
  memcpy(&m_buffer.m_header, p, copy_size);
  if (detect_toggle_endian) m_buffer.m_header.toggle_endian();

  Uint32 octets_size = m_buffer.m_header.m_octets_size;
  if (fixed_header_size + octets_size > header_size) RETURN(-1);
  if (octets_size > sizeof(m_buffer.m_octets)) RETURN(-1);
...
int ndb_ndbxfrm1::header::get_compression_method() const {
  switch (m_buffer.m_header.m_flags & fixed_header::flag_compress_method_mask) {
    case 0:
      return 0;
    case fixed_header::flag_compress_method_deflate:
...
int ndb_ndbxfrm1::header::get_compression_padding() const {
  switch (m_buffer.m_header.m_flags &
          fixed_header::flag_compress_padding_mask) {
    case 0:
      return 0;
    case fixed_header::flag_compress_padding_pkcs:
      return padding_pkcs;
    default:
      RETURN(-1);
  }
...
int ndb_ndbxfrm1::header::set_encryption_key_selection_mode(
    Uint32 key_selection_mode, Uint32 key_data_unit_size) {
  if ((m_buffer.m_header.m_flags &
       fixed_header::flag_encrypt_key_selection_mode_mask) != 0)
    RETURN(-1);  // already set
  ...
  switch (key_selection_mode) {
    case key_selection_mode_same:
      m_buffer.m_header.m_flags |=
          fixed_header::flag_encrypt_key_selection_mode_same;
      break;
    case key_selection_mode_pair:
      m_buffer.m_header.m_flags |=
          fixed_header::flag_encrypt_key_selection_mode_pair;
      break;
...
int ndb_ndbxfrm1::header::set_encryption_keying_material(
    const byte *keying_material, size_t keying_material_size,
  ...
  memcpy(m_buffer.m_octets + m_buffer.m_header.m_octets_size, keying_material,
...
int ndb_ndbxfrm1::header::get_encryption_krm_kdf_iter_count(
    Uint32 *count) const {
...
int ndb_ndbxfrm1::header::get_encryption_key_selection_mode(
  ...
  switch (m_buffer.m_header.m_flags &
          fixed_header::flag_encrypt_key_selection_mode_mask) {
...
int ndb_ndbxfrm1::header::get_encryption_keying_material(
    byte *keying_material, size_t material_space, size_t *keying_material_size,
...
int ndb_ndbxfrm1::header::write_header(ndbxfrm_output_iterator *out) const {
  ...
  if (len < m_buffer.m_header.m_magic.m_header_size) RETURN(-1);
  p = buf;
  Uint32 avail = endp - p;
  if (avail < m_buffer.m_header.m_magic.m_fixed_header_size) RETURN(-1);
  size_t write_size =
  ...
  avail = endp - p;
  if (avail < m_buffer.m_header.m_octets_size) RETURN(-1);
...
int ndb_ndbxfrm1::trailer::write_trailer(ndbxfrm_output_iterator *out,
   ...
   * buffer.
   */
  if (p - buf < (ptrdiff_t)trailer_size) {  // Fill extra
...
int ndb_ndbxfrm1::header::prepare_header_for_write() {
  memcpy(&m_buffer.m_header.m_magic.m_magic[0], ndb_ndbxfrm1::magic,
...
int ndb_ndbxfrm1::header::fixed_header::magic::validate() const {
  if (memcmp(m_magic, ndb_ndbxfrm1::magic, sizeof(ndb_ndbxfrm1::magic)) != 0)
    RETURN(-1);
  if (m_endian != native_endian_marker) RETURN(-1);
  if (m_header_size > MAX_HEADER_SIZE + MAX_OCTETS_SIZE) RETURN(-1);
  if (m_fixed_header_size > MAX_HEADER_SIZE) RETURN(-1);
  if (!is_all_zeros(&m_zeros, sizeof(m_zeros))) RETURN(-1);
  return 0;
}

int ndb_ndbxfrm1::header::fixed_header::magic::toggle_endian() {
  if (m_endian != reverse_endian_marker) RETURN(-1);
...
int ndb_ndbxfrm1::header::fixed_header::validate() const {
  ...
  if (m_magic.validate() == -1) RETURN(-1);

  if ((m_flags & flag_zeros) != 0) RETURN(-1);

  if (m_dbg_writer_ndb_version != NDB_VERSION_D) RETURN(-1);

  if (m_octets_size > MAX_OCTETS_SIZE) RETURN(-1);
  ...
  if (compress) {
    ...
    if (m_compress_dbg_writer_header_version.validate() == -1) RETURN(-1);
    if (m_compress_dbg_writer_library_version.validate() == -1) RETURN(-1);
  } else {
  ...
  if (encrypt) {
    if (m_encrypt_dbg_writer_header_version.validate() == -1) RETURN(-1);
    if (m_encrypt_dbg_writer_library_version.validate() == -1) RETURN(-1);

    Uint64 material_end = Uint64{m_encrypt_krm_keying_material_size} *
                              Uint64{m_encrypt_krm_keying_material_count} +
                          m_encrypt_krm_keying_material_position_in_octets;
    if (material_end > m_octets_size) RETURN(-1);

    switch (m_flags & flag_encrypt_cipher_mask) {
      case flag_encrypt_cipher_aes_256_cbc:
        break;
      case flag_encrypt_cipher_aes_256_xts:
        break;
      default:
        RETURN(-1);
    }
    ...
    switch (m_flags & flag_encrypt_padding_mask) {
      case flag_encrypt_padding_none:
        break;
      case flag_encrypt_padding_pkcs:
        break;
      default:
        RETURN(-1);
    }
    ...
    switch (m_flags & flag_encrypt_key_selection_mode_mask) {
      ...
      case flag_encrypt_key_selection_mode_mix_pair:
        ...
        if (m_encrypt_key_data_unit_size == 0) RETURN(-1);
        break;
      default:
        RETURN(-1);
    }
  ...
  if (!is_all_zeros(&m_zeros01, sizeof(m_zeros01))) RETURN(-1);
...
int ndb_ndbxfrm1::header::transform_version::validate() const {
  switch (m_flags & flag_product_mask) {
    case flag_product_zlib:
      break;
    case flag_product_OpenSSL:
      break;
    default:
      RETURN(-1);
  }
  switch (m_flags & flag_version_type_mask) {
    case flag_version_type_char:
      break;
    case flag_version_type_int32:
      break;
    default:
      RETURN(-1);
  }
...
int ndb_ndbxfrm1::header::fixed_header::toggle_endian() {
  ...
  if (m_compress_dbg_writer_header_version.toggle_endian() == -1) RETURN(-1);
  if (m_compress_dbg_writer_library_version.toggle_endian() == -1) RETURN(-1);

  if (m_encrypt_dbg_writer_header_version.toggle_endian() == -1) RETURN(-1);
  if (m_encrypt_dbg_writer_library_version.toggle_endian() == -1) RETURN(-1);
  ...
  static_assert(sizeof(ndb_ndbxfrm1::header::fixed_header) == 160,
                "Remember update "
                "ndb_ndbxfrm1::header::fixed_header::toggle_endian() when "
                "adding new fields.");
  return 0;
}
...
int ndb_ndbxfrm1::trailer::read_trailer(ndbxfrm_input_reverse_iterator *in) {
  ...
  if (detect_toggle_endian) {
    if (magicp->m_endian != reverse_endian_marker) RETURN(-1);
  ...
  if (detect_toggle_endian) m_buffer.m_trailer.toggle_endian();
...
int ndb_ndbxfrm1::trailer::validate_trailer() const {
  if (m_buffer.m_trailer.m_magic.validate() == -1) RETURN(-1);
  if (m_buffer.m_trailer.validate() == -1) RETURN(-1);
  return 0;
}
...
void ndb_ndbxfrm1::header::printf(FILE *out) const {
  ...
  fprintf(out, "    flag_encrypt_krm: %llu,\n",
  ...
  fprintf(out, "    flag_encrypt_key_selection_mode: %llu,\n",
          (fixed_header.m_flags &
           fixed_header::flag_encrypt_key_selection_mode_mask) >>
              24);
  ...
  fprintf(out, "    encrypt_krm_keying_material_size: %u,\n",
          fixed_header.m_encrypt_krm_keying_material_size);
  fprintf(out, "    encrypt_krm_keying_material_count: %u,\n",
  ...
  fprintf(out, "    encrypt_krm_keying_material_position_in_octets: %u,\n",

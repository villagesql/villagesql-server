...
runCreateDropConsume(NDBT_Context* ctx, NDBT_Step* step)
{
  ...
  for (Uint32 i=0; i< NumOperations; i++)
  {
    ...
    if ((ops[i] = createEventOperation(ndb, *ctx->getTab())) == NULL) {
  ...
  while (ndb->pollEvents(1000) > 0)
  {
    ...
    while ((op = ndb->nextEvent()) != NULL) {
      ...
      if (epoch != latest_epoch) {
        ...
        Uint32 iter = 0;
        Uint32 et = 0;
        while ((gciOp = ndb->getGCIEventOperations(&iter, &et)) != NULL) {

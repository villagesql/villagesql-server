    ...
    que_thr_t *thr)    /*!< in: query thread */
{
  ...
  while (index != nullptr) {
    ...
    // VillageSQL: a custom index has no B-tree, so undo of an insert must not
    // go through InnoDB's secondary-index remove path. The extension reverses
    // the maintenance through the VillageSQL custom-index runtime. Skip it the
    // same way FTS indexes are skipped.
